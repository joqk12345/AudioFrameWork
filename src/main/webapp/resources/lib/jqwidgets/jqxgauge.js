...
(function (d) {
    var b = {defineInstance: function () {
        ...
        this._r = null;
        this._animations = [];
        this.aria = {"aria-valuenow": {name: "value", type: "number"}, "aria-valuemin": {name: "min", type: "number"}, "aria-valuemax": {name: "max", type: "number"}, "aria-disabled": {name: "disabled", type: "boolean"}}
        ...
        var g = this._originalRadius, e = this._r._toLinearGradient("#ffffff", [
            [3, 2],
            [100, 1]
        ...
        var p = this._getAngleByValue(m.startValue), j = this._getAngleByValue(m.endValue), n = this._originalRadius, f = n - this._getRangeDistance(m.startDistance, m.startWidth), r = n - this._getRangeDistance(m.endDistance, m.endWidth), l = m.startWidth, e = m.endWidth, k = this._outerBorderOffset(), i = {x: n + k + f * Math.sin(p), y: n + k + f * Math.cos(p)}, q = {x: n + k + r * Math.sin(j), y: n + k + r * Math.cos(j)}, s = this._getProjectionPoint(p, n + k, f, l), o = this._getProjectionPoint(j, n + k, r, e), h = "default", t, m;
        ...
        e = this._r.measureText(n, 0, {"class": j});
        l = this._r.text(n, Math.round(m) - e.width / 2, Math.round(k), e.width, e.height, 0, {"class": this.toThemeProperty("jqx-gauge-label")});
        this._labels.push(l)
    }, _addCaption: function () {
        var l = this.caption.value, j = this.toThemeProperty("jqx-gauge-caption"), k = this.caption.offset, h = this._r.measureText(l, 0, {"class": j}), e = this._getPosition(this.caption.position, h, k), i = this.caption.style, f = this._outerBorderOffset(), g = this._r.text(l, e.left + f, e.top + f, h.width, h.height, 0, {"class": j});
        this._caption = g
        ...
        if (g <= this.max && g >= this.min) {
            ...
            if (this._isVML) {
                this._r.attr(this._pointer.childNodes[0], {v: f})
            } else {
                this._r.attr(this._pointer, {d: f})
            }
            this.value = g;
    ...
    var c = {_events: ["valueChanging", "valueChanged"], _animationTimeout: 10, _schemes: d.jqx._jqxChart.prototype.colorSchemes, _getScale: function (e, g, f) {
        ...
        var g = this._r.measureText(h, 0, {"class": this.toThemeProperty("jqx-gauge-label")}), f = this._r.measureText(e, 0, {"class": this.toThemeProperty("jqx-gauge-label")});
        if (f.width > g.width) {
            return f
        }
        return g
        ...
        var h = this._getBorderSize(), l = Math.ceil(this._valueToCoordinates(n)) + h, f = h, g = Math.ceil(this._getSize(this.ticksOffset[0], "width")), e = Math.ceil(this._getSize(this.ticksOffset[1], "height")), i = Math.ceil(this._getSize(this.pointer.offset)), m = Math.ceil(this._getMaxTickSize()), q = Math.ceil(this._getSize(this.pointer.size)), j = Math.ceil(Math.sqrt((q * q) / 3)), p, k, o;
        if (this.orientation === "vertical") {
            f += g + m + i;
            k = (i >= 0) ? f + q : f - q;
        ...
        if (this._isVML) {
            this._r.attr(this._pointer.childNodes[0], {v: e});
            this._pointer.v = e
        } else {
            this._r.attr(this._pointer, {d: e})
        }
    }, _setRectAttrs: function (e) {
        if (!this._isVML) {
            this._r.attr(this._pointer, {x: e.left});
            this._r.attr(this._pointer, {y: e.top});
            this._r.attr(this._pointer, {width: e.width});
            this._r.attr(this._pointer, {height: e.height})
        } else {
