        ...
        if (k != null && k.length > 0) {
            ...
            for (var l = 0; l < this._clipboardselection.length; l++) {
                for (var f = 0; f < this._clipboardselection[l].length; f++) {
                    ...
                    if (b != null) {
                        this._raiseEvent(17, {rowindex: j + l, datafield: i.datafield, value: b});
                        this.setcellvalue(i.row, i.column, b, false);
                        this._raiseEvent(18, {rowindex: j + l, datafield: i.datafield, oldvalue: this.getcellvalue(i.rowindex, i.datafield), value: b});
        ...
        switch (q.selectionmode) {
            ...
            case"multiplecellsadvanced":
                ...
                if (E != null) {
                    ...
                    var s = function (I, G, H) {
                        var F = function (M, J) {
                            var L = q.dataview.loadedrecords[M];
                            if (L != undefined && J != null) {
                                if (H || H == undefined) {
                                    q.clearselection()
                                }
                                var K = q.getboundindex(L);
                                q.selectcell(K, J);
                                q._oldselectedcell = q.selectedcell;
                                p = true;
                                q.ensurecellvisible(M, J);
                                return true
                            }
                            return false
                        };
                        if (!F(I, G)) {
                            q.ensurecellvisible(I, G);
                            F(I, G);
                            if (q.virtualmode) {
                                q.host.focus()
                            }
                        ...
                        if (x.shiftKey && D != 9) {
                            if (q.selectionmode == "multiplecellsextended" || q.selectionmode == "multiplecellsadvanced") {
                                if (q._lastClickedCell) {
                                    q._selectpath(I, G);
                                    q.selectedcell = {rowindex: I, datafield: G};
                                    return
                                }
                            }
                        } else {
                            if (!x.shiftKey) {
                                q._lastClickedCell = {row: I, column: G}
                            }
                        }
                    };
