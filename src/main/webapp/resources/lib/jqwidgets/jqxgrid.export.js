...
(function (a) {
    a.extend(a.jqx._jqxGrid.prototype, {exportdata: function (q, y, x, o, r, t, f) {
            ...
            if (d.cellclassname != "") {
                ...
                if (typeof d.cellclassname == "string") {
                    ...
                    for (var B = 0; B < o.length; B++) {
                        var u = this.getrowboundindex(B);
                        var b = d.cellclassname(u, d.displayfield, o[B][d.displayfield], o[B]);
                        if (b) {
                            d.customCellStyles[B] = b
                        }
                    }
                }
            }
        ...
        a.each(this.columns.records, function (J) {
            var M = a(G.table[0].rows[0].cells[J]);
            if (G.table[0].rows.length > 1) {
                var j = a(G.table[0].rows[1].cells[J])
            }
            var I = this;
            var K = function (O) {
                O.removeClass(G.toThemeProperty("jqx-grid-cell-selected"));
                O.removeClass(G.toThemeProperty("jqx-fill-state-pressed"));
                O.removeClass(G.toThemeProperty("jqx-grid-cell-hover"));
                O.removeClass(G.toThemeProperty("jqx-fill-state-hover"));
                if (I.customCellStyles) {
                    for (var P in I.customCellStyles) {
                        O.removeClass(I.customCellStyles[P])
                    }
                }
            };
            K(M);
            ...
            if (G.showaggregates) {
                if (G.getcolumnaggregateddata) {
                    h.push(G.getcolumnaggregateddata(this.displayfield, this.aggregates, true, o))
                }
            }
            var L = G._getexportcolumntype(this);
            if (this.exportable && (!this.hidden || E)) {
                D[this.displayfield] = {};
                D[this.displayfield].text = this.text;
                D[this.displayfield].width = parseInt(this.width);
                if (isNaN(D[this.displayfield].width)) {
                    D[this.displayfield].width = 60
                }
                D[this.displayfield].formatString = this.cellsformat;
                D[this.displayfield].localization = G.gridlocalization;
                D[this.displayfield].type = L;
                D[this.displayfield].cellsAlign = this.cellsalign;
                D[this.displayfield].hidden = !x
            }
            g = "cell" + e;
            ...
            if (q == "html" || q == "xls" || q == "pdf") {
                var i = function (O, W, V, P, U, R, Q, S, T) {
                    ...
                    if (U.exportable && (!U.hidden || E)) {
                        if (S == true) {
                            if (!D[U.displayfield].customCellStyles) {
                                D[U.displayfield].customCellStyles = new Array()
                            }
                            D[U.displayfield].customCellStyles[T] = O
                        } else {
                            if (V) {
                                D[U.displayfield].style = O
                            } else {
                                if (!P) {
                                    D[U.displayfield].cellStyle = O
                                } else {
                                    D[U.displayfield].cellAltStyle = O
                                }
                            }
                        }
                    }
                };
                i(F, N, true, false, this, G, J);
                c++;
                i(g, M, false, false, this, G, J);
                if (G.altrows) {
                    g = "cellalt" + e;
                    i(g, j, false, true, this, G, J)
                }
                if (this.customCellStyles) {
                    for (var H in I.customCellStyles) {
                        M.removeClass(I.customCellStyles[H])
                    }
                    for (var H in I.customCellStyles) {
                        M.addClass(I.customCellStyles[H]);
                        i(g + I.customCellStyles[H], M, false, false, this, G, J, true, H);
                        M.removeClass(I.customCellStyles[H])
                    }
                }
                e++
            ...
            if (h.length > 0) {
                a.each(this.columns.records, function (j) {
                    if (this.aggregates) {
                        for (var I = 0; I < this.aggregates.length; I++) {
                            ...
                            if (C[I]) {
                                var J = G._getaggregatename(this.aggregates[I]);
                                var K = G._getaggregatetype(this.aggregates[I]);
                                var H = h[j - k];
                                if (H) {
                                    C[I][this.displayfield] = z + J + ": " + H[K]
                                }
                            }
                        }
                    }
                ...
                a.each(this.columns.records, function (j) {
                    for (var H = 0; H < C.length; H++) {
                        if (C[H][this.displayfield] == undefined) {
                            C[H][this.displayfield] = z
                        }
                    }
                })
            }
        ...
        var m = this;
        var s = a.jqx.dataAdapter.ArrayExporter(o, D, n);
        if (y == undefined) {
            this._renderrows(this.virtualsizeinfo);
            var p = s.exportTo(q);
            if (this.showaggregates) {
