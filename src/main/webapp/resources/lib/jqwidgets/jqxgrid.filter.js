...
(function (a) {
    a.extend(a.jqx._jqxGrid.prototype, {_updatefilterrowui: function (f) {
                    ...
                    switch (g.filtertype) {
                        ...
                        case"date":
                            if (this.host.jqxDateTimeInput) {
                                a(k.children()[0]).jqxDateTimeInput({width: c - 10})
                            } else {
                                d(this, k)
                            }
                            break;
                        ...
                        case"checkedlist":
                            if (this.host.jqxDropDownList) {
                                a(k.children()[0]).jqxDropDownList({width: c - 10})
                            } else {
                                d(this, k)
                            }
                            break;
        ...
        for (var d = 0; d < b; d++) {
            ...
            var g = a(this.filterrow[0].cells[d]);
            if (c.filterable) {
                var e = function (i, j) {
                    var h = a(j.children()[0]);
                    h.val("");
                    if (h[0]) {
                        i["_oldWriteText" + h[0].id] = ""
                    }
                };
                switch (c.filtertype) {
                    case"number":
                    ...
                    case"date":
                        if (this.host.jqxDateTimeInput) {
                            a(g.children()[0]).jqxDateTimeInput("setDate", null)
                        } else {
                            e(this, g)
                        }
                        break;
                    ...
                    case"list":
                        if (this.host.jqxDropDownList) {
                            a(g.children()[0]).jqxDropDownList("clearSelection")
                        } else {
                            e(this, g)
                        }
                        break;
                    case"checkedlist":
                        if (this.host.jqxDropDownList) {
                            a(g.children()[0]).jqxDropDownList("checkAll", false)
                        } else {
                            e(this, g)
                        }
                        break;
                    ...
                    case"boolean":
                        if (!this.host.jqxCheckBox) {
                            ...
                            a(g.children()[0]).jqxCheckBox({checked: null})
                        }
                        break
                }
            }
    ...
    }, _addfilterwidget: function (A, d, y) {
        var E = this.that;
        var x = "";
        for (var C = 0; C < E.dataview.filters.length; C++) {
            var v = E.dataview.filters[C];
            if (v.datafield && v.datafield == A.datafield) {
                x = v.filter.getfilters()[0].value;
                break
            }
        }
        var g = function (F, G) {
            ...
            f.addClass(F.toThemeProperty("jqx-widget"));
            f.addClass(F.toThemeProperty("jqx-input"));
            f.addClass(F.toThemeProperty("jqx-widget-content"));
            if (F.rtl) {
                f.css("direction", "rtl")
            }
            f.appendTo(G);
            f.width(y - 10);
            f.height(F.filterrowheight - 10);
            f.css("margin", "4px");
            if (A.createfilterwidget) {
                A.createfilterwidget(A, G, f)
            }
            A._filterwidget = f;
            f.focus(function () {
                ...
                F.focusedfilter = f;
                f.addClass(F.toThemeProperty("jqx-fill-state-focus"));
                return false
            });
            f.blur(function () {
                f.removeClass(F.toThemeProperty("jqx-fill-state-focus"))
            });
            f.keydown(function (H) {
                if (H.keyCode == "13") {
                    F._applyfilterfromfilterrow()
                }
                if (f[0]._writeTimer) {
                ...
                f[0]._writeTimer = setTimeout(function () {
                    if (!F._loading) {
                        if (F["_oldWriteText" + f[0].id] != f.val()) {
                            F._applyfilterfromfilterrow();
                            F["_oldWriteText" + f[0].id] = f.val()
                        }
                    }
                }, 800);
                F.focusedfilter = f
            });
            F.host.removeClass("jqx-disableselect");
            F.content.removeClass("jqx-disableselect");
            f.val(x)
        };
        if (A.datatype != null) {
            if (A.filtertype == "number") {
                if (A.datatype == "string" || A.datatype == "date" || A.datatype == "bool") {
                    A.filtertype = "textbox"
                }
            }
            if (A.filtertype == "date") {
                if (A.datatype == "string" || A.datatype == "number" || A.datatype == "bool") {
                    A.filtertype = "textbox"
                }
            }
            if (A.filtertype == "bool") {
                if (A.datatype == "string" || A.datatype == "number" || A.datatype == "date") {
                    A.filtertype = "textbox"
                }
            }
        }
        switch (A.filtertype) {
            case"number":
                var l = a("<div></div>");
                l.width(d.width());
                l.height(this.filterrowheight);
                d.append(l);
                var y = d.width() - 20;
                var q = function (G, H, f) {
                    var F = a('<input style="float: left;" autocomplete="off" type="textarea"/>');
                    if (E.rtl) {
                        F.css("float", "right");
                        F.css("direction", "rtl")
                    }
                    F[0].id = a.jqx.utilities.createId();
                    F.addClass(E.toThemeProperty("jqx-widget"));
                    F.addClass(E.toThemeProperty("jqx-input"));
                    F.addClass(E.toThemeProperty("jqx-widget-content"));
                    F.appendTo(G);
                    F.width(H - 10);
                    F.height(E.filterrowheight - 10);
                    F.css("margin", "4px");
                    F.css("margin-right", "2px");
                    F.focus(function () {
                        E.focusedfilter = F;
                        F.addClass(E.toThemeProperty("jqx-fill-state-focus"))
                    });
                    F.blur(function () {
                        F.removeClass(E.toThemeProperty("jqx-fill-state-focus"))
                    });
                    F.keydown(function (I) {
                        if (I.keyCode == "13") {
                            E._applyfilterfromfilterrow()
                        }
                        if (F[0]._writeTimer) {
                            clearTimeout(F[0]._writeTimer)
                        }
                        F[0]._writeTimer = setTimeout(function () {
                            if (!E._loading) {
                                if (E["_oldWriteText" + F[0].id] != F.val()) {
                                    E._applyfilterfromfilterrow();
                                    E["_oldWriteText" + F[0].id] = F.val()
                                }
                            }
                        }, 800);
                        E.focusedfilter = F
                ...
                q(l, y);
                var z = E._getfiltersbytype("number");
                var r = a("<div class='filter' style='float: left;'></div>");
                r.css("margin-top", "4px");
                r.appendTo(l);
                if (E.rtl) {
                    r.css("float", "right")
                }
                var h = 0;
                if (A.filtercondition != null) {
                    var e = z.indexOf(A.filtercondition);
                    if (e != -1) {
                        h = e
                    }
                }
                r.jqxDropDownList({touchMode: E.touchmode, rtl: E.rtl, dropDownHorizontalAlignment: "right", enableBrowserBoundsDetection: true, selectedIndex: h, width: 18, height: 20, dropDownHeight: 150, dropDownWidth: 170, source: z, theme: E.theme});
                r.jqxDropDownList({selectionRenderer: function (f) {
                    return""
                }});
                r.jqxDropDownList("setContent", "");
                r.find(".jqx-dropdownlist-content").hide();
                if (A.createfilterwidget) {
                    A.createfilterwidget(A, d, l)
                }
                A._filterwidget = l;
                var j = null;
                this.addHandler(r, "select", function () {
                    var f = r.jqxDropDownList("getSelectedItem").label;
                    if (A._filterwidget.find("input").val().length > 0 && !E.refreshingfilter) {
                        E._applyfilterfromfilterrow()
                    } else {
                        if (A._filterwidget.find("input").val().length == 0 && !E.refreshingfilter) {
            ...
            case"date":
                if (this.host.jqxDateTimeInput) {
                    ...
                    b.css("margin", "4px");
                    b.appendTo(d);
                    var m = {calendar: this.gridlocalization, todayString: this.gridlocalization.todaystring, clearString: this.gridlocalization.clearstring};
                    b.jqxDateTimeInput({localization: m, rtl: E.rtl, showFooter: true, formatString: A.cellsformat, selectionMode: "range", value: null, theme: this.theme, width: y - 10, height: this.filterrowheight - 10});
                    if (A.createfilterwidget) {
                        A.createfilterwidget(A, d, b)
                    }
                    A._filterwidget = b;
                    this.addHandler(b, "valuechanged", function (f) {
                        if (!E.refreshingfilter) {
                            E._applyfilterfromfilterrow();
                            E.focusedfilter = null
                        }
                    })
                } else {
                    g(this, d)
            ...
            case"checkedlist":
                if (this.host.jqxDropDownList) {
                    var p = this._getfilterdataadapter(A);
                    var k = false;
                    var r = a("<div></div>");
                    r.css("margin", "4px");
                    var s = A.datafield;
                    var t = A.filtertype == "checkedlist" ? true : false;
                    var B = y < 150 ? 220 : "auto";
                    r.jqxDropDownList({touchMode: E.touchmode, rtl: E.rtl, checkboxes: t, dropDownWidth: B, source: p, autoDropDownHeight: k, theme: this.theme, width: y - 10, height: this.filterrowheight - 10, displayMember: A.displayfield, valueMember: s});
                    r.appendTo(d);
                    var o = r.jqxDropDownList("getItems");
                    var c = r.jqxDropDownList("listBox");
                    if (o.length < 8) {
                        r.jqxDropDownList("autoDropDownHeight", true)
                    } else {
                        r.jqxDropDownList("autoDropDownHeight", false);
                        k = false
                    }
                    if (t) {
                        r.jqxDropDownList({selectionRenderer: function () {
                            return E.gridlocalization.filterselectstring
                        }});
                        var w = a('<span style="top: 2px; position: relative; color: inherit; border: none; background-color: transparent;">' + E.gridlocalization.filterselectstring + "</span>");
                        w.addClass(this.toThemeProperty("jqx-item"));
                        if (c != undefined) {
                            if (!k) {
                                c.host.height(200)
                            }
                            c.insertAt(E.gridlocalization.filterselectallstring, 0);
                            r.jqxDropDownList("setContent", w);
                            var i = true;
                            var D = new Array();
                            c.checkAll(false);
                            E.addHandler(c.host, "checkChange", function (G) {
                                r[0]._selectionChanged = true;
                                if (!i) {
                                    return
                                }
                                if (G.args.label != E.gridlocalization.filterselectallstring) {
                                    i = false;
                                    c.host.jqxListBox("checkIndex", 0, true, false);
                                    var f = c.host.jqxListBox("getCheckedItems");
                                    var F = c.host.jqxListBox("getItems");
                                    if (f.length == 1) {
                        ...
                        r.jqxDropDownList({selectedIndex: 0})
                    }
                    if (A.createfilterwidget) {
                        A.createfilterwidget(A, d, r)
                    }
                    A._filterwidget = r;
                    var n = r.jqxDropDownList("dropdownlistWrapper");
                    if (A.filtertype == "list") {
                        this.addHandler(r, "select", function (f) {
                            if (!E.refreshingfilter) {
                        ...
                        this.addHandler(r, "close", function (f) {
                            if (r[0]._selectionChanged) {
                                E._applyfilterfromfilterrow();
                                E.focusedfilter = null;
                                r[0]._selectionChanged = false
                            }
                        })
                    }
                } else {
            ...
            case"boolean":
                if (this.host.jqxCheckBox) {
                    var u = a('<div tabIndex=0 style="opacity: 0.99; position: absolute; top: 50%; left: 50%; margin-top: -7px; margin-left: -10px;"></div>');
                    u.appendTo(d);
                    u.jqxCheckBox({enableContainerClick: false, animationShowDelay: 0, animationHideDelay: 0, hasThreeStates: true, theme: this.theme, checked: null});
                    if (A.createfilterwidget) {
                        A.createfilterwidget(A, d, u)
                    }
                    if (x === true || x == "true") {
                        u.jqxCheckBox({checked: true})
                    } else {
                        if (x === false || x == "false") {
                            u.jqxCheckBox({checked: false})
                        }
                    }
                    A._filterwidget = u;
                    this.addHandler(u, "change", function (f) {
                        if (!E.refreshingfilter) {
                            if (f.args) {
                                E.focusedfilter = null;
                                E._applyfilterfromfilterrow()
    ...
    }, _updatelistfilters: function (n, t) {
        var o = this.that;
        ...
        for (var h = 0; h < r; h++) {
            ...
            if (e.filterable) {
                if (e.filtertype == "list" || e.filtertype == "checkedlist") {
                    var m = e._filterwidget;
                    if (!n) {
                        if (e.filter == undefined) {
                            m.jqxDropDownList("renderSelection");
                            continue
                        }
                    } else {
                        var b = this._getfilterdataadapter(e);
                        m.jqxDropDownList({source: b});
                        var g = m.jqxDropDownList("getItems");
                        var s = true;
                        if (g.length != b.records.length + 1) {
                            s = false
                        }
                        if (s) {
                            for (var k = 1; k < g.length; k++) {
                                if (g[k].label != b.records[k - 1][e.displayfield]) {
                                    s = false;
                                    break
                                }
                            }
                    ...
                    var l = e.filtertype == "checkedlist" ? true : false;
                    var g = m.jqxDropDownList("getItems");
                    var c = m.jqxDropDownList("listBox");
                    m.jqxDropDownList("dataBind");
                    if (l) {
                        m.jqxDropDownList({selectionRenderer: function () {
                            return o.gridlocalization.filterselectstring
                        }});
                        c.insertAt(this.gridlocalization.filterselectallstring, 0);
                        m.jqxDropDownList("setContent", this.gridlocalization.filterselectstring);
                        c.checkAll(false);
                        if (e.filter) {
                            var d = e.filter.getfilters();
                            for (var k = 0; k < c.items.length; k++) {
                                var q = c.items[k].label;
                                a.each(d, function () {
                                    if (this.condition == "NOT_EQUAL") {
                                        if (q == this.value) {
                                            c.uncheckIndex(k, false, false)
                                        } else {
                                            c.checkIndex(k, false, false)
                                        }
                                    } else {
                            ...
                            c._updateCheckedItems();
                            var p = c.getCheckedItems().length;
                            if (c.items.length != p && p > 0) {
                                c.host.jqxListBox("indeterminateIndex", 0, true, false)
                            }
                        }
                    } else {
                    ...
                    if (g.length < 8) {
                        m.jqxDropDownList("autoDropDownHeight", true)
                    } else {
                        m.jqxDropDownList("autoDropDownHeight", false)
                    }
                }
            }
        }
        ...
        return d
    }, _updatefilterpanel: function (K, e, f) {
        if (K == null || K == undefined) {
            K = this
        }
        var g = K._getcolumntypebydatafield(f);
        var C = K._getfiltersbytype(g);
        if (!K.host.jqxDropDownList) {
            throw new Error("jqxGrid: Missing reference to jqxdropdownlist.js.");
            return
        }
        var A = a(e);
        var w = A.find("#filterclearbutton" + K.element.id);
        var l = A.find("#filterbutton" + K.element.id);
        var F = A.find("#filter1" + K.element.id);
        var B = A.find("#filter2" + K.element.id);
        var D = A.find("#filter3" + K.element.id);
        var z = A.find(".filtertext1" + K.element.id);
        var y = A.find(".filtertext2" + K.element.id);
        z.val("");
        y.val("");
        this.removeHandler(l, "click");
        this.addHandler(l, "click", function () {
            K._buildfilter(K, e, f);
            K._closemenu()
        });
        this.removeHandler(w, "click");
        this.addHandler(w, "click", function () {
            K._clearfilter(K, e, f);
            K._closemenu()
        });
        if (this.filtermode === "default") {
            if (F.jqxDropDownList("source") != C) {
                F.jqxDropDownList({enableBrowserBoundsDetection: false, source: C});
                D.jqxDropDownList({enableBrowserBoundsDetection: false, source: C})
            }
            if (g == "boolean" || g == "bool") {
                F.jqxDropDownList({autoDropDownHeight: true, selectedIndex: 0});
                D.jqxDropDownList({autoDropDownHeight: true, selectedIndex: 0})
            } else {
                var h = false;
                if (C && C.length) {
                    if (C.length < 5) {
                ...
                D.jqxDropDownList({autoDropDownHeight: h, selectedIndex: 2})
            }
            B.jqxDropDownList({selectedIndex: 0});
            var u = f.filter;
            ...
            switch (g) {
                ...
                case"decimal":
                    d = "numericfilter";
                    n = o.getoperatorsbyfiltertype("numericfilter");
                    break;
                case"boolean":
                case"bool":
                    d = "booleanfilter";
                    n = o.getoperatorsbyfiltertype("booleanfilter");
                    break;
                case"date":
                case"time":
                    d = "datefilter";
                    n = o.getoperatorsbyfiltertype("datefilter");
                    break;
                case"string":
                    d = "stringfilter";
                    n = o.getoperatorsbyfiltertype("stringfilter");
                    break
            }
            if (u != null) {
                var H = u.getfilterat(0);
                var G = u.getfilterat(1);
                var s = u.getoperatorat(0);
                if (K.updatefilterconditions) {
                    var n = [];
                    var L = K.updatefilterconditions(d, n);
                    if (L != undefined) {
                        for (var I = 0; I < L.length; I++) {
                            L[I] = L[I].toUpperCase()
                        }
                        u.setoperatorsbyfiltertype(d, L);
                        n = L
                    }
                }
                var r = this.enableanimations ? "default" : "none";
                if (H != null) {
                    var k = n.indexOf(H.comparisonoperator);
                    var x = H.filtervalue;
                    z.val(x);
                    F.jqxDropDownList({selectedIndex: k, animationType: r})
                }
                if (G != null) {
                    var j = n.indexOf(G.comparisonoperator);
                    var v = G.filtervalue;
                    y.val(v);
                    D.jqxDropDownList({selectedIndex: j, animationType: r})
                }
                if (u.getoperatorat(0) == undefined) {
                    B.jqxDropDownList({selectedIndex: 0, animationType: r})
                } else {
                    if (u.getoperatorat(0) == "and" || u.getoperatorat(0) == 0) {
                        B.jqxDropDownList({selectedIndex: 0})
                    } else {
                        B.jqxDropDownList({selectedIndex: 1})
                    }
                }
            }
            if (K.updatefilterpanel) {
                K.updatefilterpanel(F, D, B, z, y, l, w, u, d, n)
            }
            z.focus();
            setTimeout(function () {
                z.focus()
            ...
            var m = K._getfilterdataadapter(f);
            var d = K._getfiltertype(g);
            if (f.cellsformat) {
                F.jqxListBox({displayMember: f.displayfield, valueMember: f.displayfield + "JQValue", source: m})
            } else {
                F.jqxListBox({displayMember: f.displayfield, valueMember: f.displayfield, source: m})
            }
            F.jqxListBox("insertAt", K.gridlocalization.filterselectallstring, 0);
            var b = F.data().jqxListBox.instance;
            b.checkAll(false);
            var p = this;
            if (f.filter) {
                b.uncheckAll(false);
                ...
                for (var E = 0; E < b.items.length; E++) {
                    var q = b.items[E].value;
                    a.each(t, function () {
                        if (this.condition == "NOT_EQUAL") {
                            if (q != this.value) {
                                b.uncheckIndex(E, false, false);
                                return false
                            }
                        } else {
                            if (this.condition == "EQUAL") {
                                if (q == this.value) {
                                    b.checkIndex(E, false, false);
                                    return false
                                }
                            }
                        }
                ...
                var J = b.getCheckedItems().length;
                if (b.items.length != J && J > 0) {
                    b.host.jqxListBox("indeterminateIndex", 0, true, false)
                }
                if (J === b.items.length - 1) {
                    b.host.jqxListBox("checkIndex", 0, true, false)
                }
            }
        }
    ...
    }, _initfilterpanel: function (v, b, c, n) {
        if (v == null || v == undefined) {
            v = this
        }
        b[0].innerHTML = "";
        var r = a("<div class='filter' style='margin-left: 7px;'></div>");
        b.append(r);
        var m = a("<div class='filter' style='margin-top: 3px; margin-bottom: 3px;'></div>");
        m.text(v.gridlocalization.filtershowrowstring);
        var s = a("<div class='filter' id='filter1" + v.element.id + "'></div>");
        var h = a("<div class='filter' id='filter2" + v.element.id + "' style='margin-bottom: 3px;'></div>");
        var q = a("<div class='filter' id='filter3" + v.element.id + "'></div>");
        var e = v._getcolumntypebydatafield(c);
        if (!s.jqxDropDownList) {
            throw new Error("jqxGrid: jqxdropdownlist.js is not loaded.");
            return
        }
        var o = v._getfiltersbytype(e);
        var j = a("<div class='filter'><input class='filtertext1" + v.element.id + "' style='height: 20px; margin-top: 3px; margin-bottom: 3px;' type='text'></input></div>");
        var l = j.find("input");
        l.addClass(this.toThemeProperty("jqx-input"));
        l.addClass(this.toThemeProperty("jqx-widget-content"));
        l.addClass(this.toThemeProperty("jqx-rc-all"));
        l.width(n - 15);
        var k = a("<div class='filter'><input class='filtertext2" + v.element.id + "' style='height: 20px; margin-top: 3px;' type='text'></input></div>");
        var i = k.find("input");
        i.addClass(this.toThemeProperty("jqx-input"));
        i.addClass(this.toThemeProperty("jqx-widget-content"));
        ...
        var f = a('<span tabIndex=0 id="filterbutton' + v.element.id + '" class="filterbutton" style="padding: 4px 12px; margin-left: 2px;">' + v.gridlocalization.filterstring + "</span>");
        g.append(f);
        var t = a('<span tabIndex=0 id="filterclearbutton' + v.element.id + '" class="filterclearbutton" style="padding: 4px 12px; margin-left: 5px;">' + v.gridlocalization.filterclearstring + "</span>");
        g.append(t);
        f.jqxButton({height: 20, theme: v.theme});
        t.jqxButton({height: 20, theme: v.theme});
        var u = function (x) {
            if (x) {
                if (x.text().indexOf("case sensitive") != -1) {
                    var w = x.text();
                    w = w.replace("case sensitive", "match case");
                    x.text(w)
                }
                x.css("font-family", v.host.css("font-family"));
                x.css("font-size", v.host.css("font-size"));
                return x
            }
            return""
        };
        if (this.filtermode === "default") {
            r.append(m);
            r.append(s);
            s.jqxDropDownList({_checkForHiddenParent: false, rtl: v.rtl, enableBrowserBoundsDetection: false, selectedIndex: 2, width: n - 15, height: 20, dropDownHeight: 150, dropDownWidth: n - 15, selectionRenderer: u, source: o, theme: v.theme});
            r.append(j);
            var p = new Array();
            p[0] = v.gridlocalization.filterandconditionstring;
            p[1] = v.gridlocalization.filterorconditionstring;
            h.jqxDropDownList({_checkForHiddenParent: false, rtl: v.rtl, enableBrowserBoundsDetection: false, autoDropDownHeight: true, selectedIndex: 0, width: 60, height: 20, source: p, selectionRenderer: u, theme: v.theme});
            r.append(h);
            q.jqxDropDownList({_checkForHiddenParent: false, rtl: v.rtl, enableBrowserBoundsDetection: false, selectedIndex: 2, width: n - 15, height: 20, dropDownHeight: 150, dropDownWidth: n - 15, selectionRenderer: u, source: o, theme: v.theme});
            r.append(q);
            r.append(k)
        } else {
            r.append(m);
            r.append(s);
            s.jqxListBox({rtl: v.rtl, _checkForHiddenParent: false, checkboxes: true, selectedIndex: 2, width: n - 15, height: 120, theme: v.theme});
            var d = true;
            v.addHandler(s, "checkChange", function (y) {
                if (!d) {
                    return
                }
                if (y.args.label != v.gridlocalization.filterselectallstring) {
                    d = false;
                    s.jqxListBox("checkIndex", 0, true, false);
                    var w = s.jqxListBox("getCheckedItems");
                    var x = s.jqxListBox("getItems");
                    if (w.length == 1) {
                        s.jqxListBox("uncheckIndex", 0, true, false)
                    } else {
                        if (x.length != w.length) {
                            s.jqxListBox("indeterminateIndex", 0, true, false)
                        }
                    }
                    d = true
                } else {
                    d = false;
                    if (y.args.checked) {
                        s.jqxListBox("checkAll", false)
                    } else {
                        s.jqxListBox("uncheckAll", false)
                    }
                    d = true
                }
            })
        ...
        r.append(g);
        if (v.updatefilterpanel) {
            v.updatefilterpanel(s, q, h, j, k, f, t, null, null, o)
        }
    }})
})(jQuery);